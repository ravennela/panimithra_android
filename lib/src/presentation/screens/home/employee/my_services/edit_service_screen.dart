        ...
        child: BlocConsumer<ServiceBloc, ServiceState>(
          listener: (context, state) {
            if (state is ServiceByIdLoaded) {
              _serviceNameController.text = state.service.serviceName;
              _descriptionController.text = state.service.description;
              _priceController.text = state.service.price.toString();
              addressController.text = state.service.address ?? '';
              addInfoLine1Controller.text = state.service.addInfoOne ?? '';
              addInfoLine2Controller.text = state.service.addInfoTwo ?? '';
              addInfoLine3Controller.text = state.service.addInfoThree ?? '';

              if (state.service.duration != null) {
                String d = timeToDuration(state.service.duration!);
                if (['30 min', '1 hour', '2 hours', 'Half day', 'Full day']
                    .contains(d)) {
                  _selectedDuration = d;
                } else {
              ...
              if (state.service.timeIn != null) {
                startTime = state.service.timeIn;
              }

              if (state.service.timeOut != null) {
                endTime = state.service.timeOut;
              }

              selectedDays.clear();
              selectedDays.addAll(state.service.datesSelected);

              if (state.service.categoryName != null) {
                categoryController.text = state.service.categoryName ?? '';
                _selectedCategory = state.service.categoryId;
                // Fetch subcategories for the selected category
                context.read<SubcategoryBloc>().add(FetchSubcategoriesEvent(
                    categoryId: _selectedCategory.toString(), page: 0));
              }

              if (state.service.subCategoryName != null) {
                subCategoryController.text =
                    state.service.subCategoryName ?? '';
                _selectedSubcategory = state.service.sabCategoryId;
              }

              if (state.service.imageUrl != null) {
                _existingIconUrl = state.service.imageUrl;
              }
