  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<CategoriesBloc, CategoriesState>(
        buildWhen: (previous, current) => ((current is FetchCategoryByIdError ||
            current is FetchCategoryByIdLoaded ||
            current is FetchCategoryByIdLoading ||
            current is UpdateCategoryError ||
            current is UpdateCategoryLoaded)),
        listener: (context, state) {
          if (state is FetchCategoryByIdLoaded) {
            _categoryNameController.text =
                state.fetchCategoryByIdModel.categoryName;
            _descriptionController.text =
                state.fetchCategoryByIdModel.description;
            _iconUrlController.text = state.fetchCategoryByIdModel.iconUrl;
            _selectedStatus = state.fetchCategoryByIdModel.status;
          }
        },
        builder: (context, state) {
          if (state is FetchCategoryByIdLoading) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          }
          if (state is FetchCategoryByIdError) {
            return Center(
              child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: BlocConsumer<CategoriesBloc, CategoriesState>(
                        listener: (context, state) {
                          if (state is UpdateCategoryError) {
                            ToastHelper.showToast(
                                context: context,
                                type: 'error',
                                title: state.message);
                          }
                          if (state is UpdateCategoryLoaded) {
                            ToastHelper.showToast(
                                context: context,
                                type: 'success',
                        ...
                        builder: (context, state) {
                          return ElevatedButton(
                            onPressed: () {
                              ...
                              context
                                  .read<CategoriesBloc>()
                                  .add(UpdateCategoryEvent(data: data));
